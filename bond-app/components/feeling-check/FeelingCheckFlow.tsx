  ...
  async function buildSummary() {
    ...
    try {
      const res = await fetch("/api/feeling-check", {
        ...
        body: JSON.stringify({ mood, intensity, triggers: Array.from(triggers),
needs: Array.from(needs), journal }),
      });
  ...
  async function sendToPartner() {
    ...
    await supabase.from("check_ins").insert({
      user_id: user.id, mood, intensity,
      triggers: [...triggers], needs: [...needs],
      journal, translation, sent_to_partner: true,
    });
    setSent(true);
  }
